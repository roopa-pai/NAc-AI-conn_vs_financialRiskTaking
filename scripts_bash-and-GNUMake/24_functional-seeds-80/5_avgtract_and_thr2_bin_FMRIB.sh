...
#23_DTI
echo 'Objective:
(1)create average tracts from FDT_THRESH_XX_DTItoFMRIB.nii.gz
...
#(0) create list of values chosen for threshold
...
do
	...
	do
		...
		then
		    ...
		    do
			if [[ -n `echo $l | grep -P FDT_THRESH_[0-9]{2}_DTItoFMRIB_bin.nii.gz` ]]
			then
...
do
	...
	do
	    ...
	    then
		mkdir -p $ResultsFolder/extr_vals/Roopa/{FA,MD,AxD,RD}/${i}/{ring0,ring1}/thr1_${thresh1}_thr2_${thresh2}
